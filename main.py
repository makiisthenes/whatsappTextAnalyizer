...
def eng_analysis(eng_word, user):  # to be continued, will add spell checking, add user parameter.
    ...
    cd_path = os.getcwd() + r'\langAnalysis\english-words.txt'
                    ...
                    if devOption:
                        if part_count >= 5 and message_start == 5:
                            ...
                            for word_contents in message_contents:  # this list is going to be analysed, word be word...
                                eng_analysis(word_contents,
                                             user)  # we also need a parameter for who this text belongs to.
                                # check dictionary of words json file...
                                pass
                        if part_count == line_total_part_counter:  # at the end of the message the list is reset back to initial.
                            message_contents = []
...
while usr_input != 1 and not hasCompleted:
    ...
    try:
        try:
            ...
            print('This mode goes through 300 thousand words for each word in the ' +
                  'message and will take a long time to complete.')
            if dev_opt == 'ENABLE':
                devOption = True
            else:
                devOption = False
